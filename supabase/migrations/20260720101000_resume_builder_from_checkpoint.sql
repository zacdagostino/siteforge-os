...
begin
  ...
  if not exists (
    ...
    raise exception 'No private source checkpoint is available for this build.';
  end if;
  ...
  set
    ...
    progress_detail = 'Resuming from the latest private source checkpoint.',
    error_summary = null,
    failure_code = null,
    failure_stage = null,
    failure_action = null,
  ...
  values (
    ...
    'note',
    'Private redesign preview resumed from its latest protected source checkpoint.'
  );
...
grant execute on function public.resume_website_build(uuid) to authenticated;
